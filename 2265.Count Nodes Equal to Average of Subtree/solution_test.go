...
func averageOfSubtree(root *TreeNode) int {
	...
	var postorder func(node *TreeNode) (int, int)
	postorder = func(node *TreeNode) (int, int) {
		if nil == node {
			return 0, 0
		}

		leftSum, leftCnt := postorder(node.Left)
		rightSum, rightCnt := postorder(node.Right)
...
func TestCountNodesEqualToAverageOfSubtree(t *testing.T) {
