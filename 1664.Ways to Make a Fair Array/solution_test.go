...
func waysToMakeFair(nums []int) int {
	...
	for k, v := range nums {
		if k%2 == 0 {
			if preEven+(oddSum-preOdd) == preOdd+(evenSum-preEven-v) {
				ans++
			}
			// 累计遍历过的偶数下标前缀和
			preEven += v
		} else {
			if preEven+(oddSum-preOdd-v) == preOdd+(evenSum-preEven) {
				ans++
			}
			// 累计遍历过的奇数下标前缀和
			preOdd += v
...
func TestWaysToMakeAFairArray(t *testing.T) {
